...
{   
    ...
    public float speedWaste;
    public float sizeWaste;
    ...
    void Awake()
    {
        ...
        redblobPrefab.GetComponent<Blob>().walls = GameObject.FindGameObjectsWithTag("wall");
         int[] blobSpeed = new int[16]; 
         int[] blobSpeed2= new int[18]; 
        WindowGraph.GetComponent<Graph>().ShowGraph("graphContainer2", blobSpeed2);
        WindowGraph.GetComponent<Graph>().ShowGraph("graphContainer", blobSpeed);
    }

    // Start is called before the first frame update
    void Start()
    {
        // I use a coroutine so I can execute functions with pause s inbetween
    ...
    public int[] createListScale(List<GameObject> blobs){
        int[] blobSpeed = new int[18]; 
        foreach (GameObject blob in blobs.ToArray())
        {
            blobSpeed[(int)(blob.transform.lossyScale.x*10)]++;
        }
        return blobSpeed;
    }
    public int[] createListVelocitat(List<GameObject> blobs){
        int[] blobSpeed = new int[16]; 
        foreach (GameObject blob in blobs.ToArray())
        {
            blobSpeed[(int)blob.GetComponent<Blob>().speed*2/10]++;
        }
        return blobSpeed;
    }
    IEnumerator Setup()
    {
        ...
        while (true)
        {
            ...
            foreach (GameObject blob in blobs.ToArray())
            {
                if (blob.GetComponent<Blob>().eatenFood > 1)
                {
                    if (blob.name == "Blob(Clone)")
                    {
                        ...
                        newblob.GetComponent<Blob>().speed = Random.Range(blob.GetComponent<Blob>().speed-3, blob.GetComponent<Blob>().speed+3);
                        float newScale = Random.Range(blob.transform.lossyScale.x-0.3f, blob.transform.lossyScale.x+0.3f);
                        newblob.transform.localScale = new Vector3(newScale, newScale, newScale);
                        newblob.GetComponent<Blob>().senseRadius = 5f*(newScale*radiusGain);
                        float enloss = (newblob.GetComponent<Blob>().speed*speedWaste)*(newScale*sizeWaste)/10f;
                        if(enloss<1){
                            newblob.GetComponent<Blob>().energyLoss = 1;
                        }else{
                            newblob.GetComponent<Blob>().energyLoss = enloss;
                        }
                        blobs.Add(newblob);
                    }
                    else
            ...
            WindowGraph.GetComponent<Graph>().ShowGraph("graphContainer2", createListScale(blobs));
            WindowGraph.GetComponent<Graph>().ShowGraph("graphContainer", createListVelocitat(blobs));
